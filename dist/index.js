...
import { z } from "zod";
var users, tickets, subscriptions, products, whatsappSettings, sentMessages, receivedMessages, aiTokenSettings, userSubscriptions, categories, carts, cartItems, addresses, orders, orderItems, transactions, insertUserSchema, insertSubUserSchema, insertTicketSchema, insertSubscriptionSchema, insertProductSchema, insertWhatsappSettingsSchema, insertSentMessageSchema, insertReceivedMessageSchema, insertAiTokenSettingsSchema, insertUserSubscriptionSchema, insertCategorySchema, insertCartSchema, insertCartItemSchema, insertAddressSchema, updateAddressSchema, insertOrderSchema, insertOrderItemSchema, insertTransactionSchema, updateCategoryOrderSchema, ticketReplySchema, resetPasswordSchema;
...
import { eq, sql as sql2, desc, and, gte, or, inArray } from "drizzle-orm";
...
var init_storage = __esm({
  "server/storage.ts"() {
    ...
    MemStorage = class {
      ...
      async getNewOrdersCount(sellerId) {
        const sellerOrders = Array.from(this.orders.values()).filter((order) => order.sellerId === sellerId && order.status === "pending");
        return sellerOrders.length;
      }
      async getUnshippedOrdersCount(sellerId) {
        const unshippedStatuses = ["pending", "confirmed", "preparing"];
        const sellerUnshippedOrders = Array.from(this.orders.values()).filter((order) => order.sellerId === sellerId && unshippedStatuses.includes(order.status));
        return sellerUnshippedOrders.length;
      }
      // Order Items
      async getOrderItems(orderId) {
...
\u0628\u0631\u0627\u06CC \u06A9\u0645\u06A9 \u0648 \u0631\u0627\u0647\u0646\u0645\u0627\u06CC\u06CC\u060C \u0645\u06CC\u200C\u062A\u0648\u0627\u0646\u06CC\u062F \u0647\u0631 \u0632\u0645\u0627\u0646 \u067E\u06CC\u0627\u0645 \u0628\u062F\u0647\u06CC\u062F.`;
  ...
  app2.post("/api/transactions", authenticateToken, requireLevel2, async (req, res) => {
    try {
  ...
  app2.get("/api/balance", authenticateToken, requireLevel2, async (req, res) => {
    try {
